...
def fetch_fred_csv(series_id: str, timeout: int = 30, retries: int = 3) -> pd.DataFrame | None:
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if attempt < retries:
                logger.debug("FRED %s fetch attempt %d failed: %s. Retrying in 5s...", series_id, attempt + 1, exc)
