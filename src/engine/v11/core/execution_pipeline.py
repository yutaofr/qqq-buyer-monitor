...
def apply_beta_floor(*, pre_floor_beta: float, floor: float = 0.5, overlay_state: str | None = None) -> tuple[float, bool]:
...
def compute_deployment_readiness(*, effective_entropy: float, e_sharpe: float, erp_percentile: float) -> float:
    ...
    return float(
        np.clip((1.0 - effective_entropy) * max(0.0, e_sharpe) * erp_percentile, 0.0, 1.0)
    )
