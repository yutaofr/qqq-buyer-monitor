...
class BlackScholesPricer:
    ...
    def price_put(self, S: float, K: float, T: float, sigma: float) -> float:
        ...
        put_price = K * np.exp(-self.r * T) * si.norm.cdf(-d2, 0.0, 1.0) - S * si.norm.cdf(
            -d1, 0.0, 1.0
        )
        return float(put_price)

    def simulate_otm_put_roll(
        self, current_price: float, current_vix: float, otm_pct: float = 0.20, dte_days: int = 180
    ) -> dict:
        """
        ...
        return {"strike": strike, "T": T, "premium": premium}
