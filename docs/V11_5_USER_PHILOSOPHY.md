...
基于 `2018-01-01` 至 `2026-03-27` 的 `3012` 个因果窗口审计，系统交出了如下答卷：

| 指标 | 审计结果 | 含义 |
| :--- | :--- | :--- |
| **制度匹配度 (Top-1 Accuracy)** | **98.71%** | 系统识别出的制度与历史基准标签具有极高一致性。 |
| **预测保真度 (Brier Score)** | **0.0225** | 衡量概率预测的“诚实度”，越接近 0 说明预测越精准。 |
| **平均决策熵值 (Mean Entropy)** | **0.046** | 系统在长周期内维持了很高的决策清晰度，不拖泥带水。 |
| **结算锁发生率 (Lock Incidence)** | **0.4%** | 行为守卫抑制无效换手，但没有把系统锁成“僵尸”。 |
...
为了让高阶使用者理解系统“全量概率”的底座，本章将解构系统两项核心底层技术：**GaussianNB 的 JIT 似然标定**与**马氏距离几何防护**。

### 9.1 GaussianNB JIT 似然标定：因果实时重训的“制度 DNA 匹配”

系统不会把一组静态参数焊死在代码里。相反，它会在每个审计或生产运行时，使用当下可见的历史 DNA 重新拟合 `GaussianNB(var_smoothing=1e-2)`，把“制度中心”和“容错宽度”都交给真实历史去决定。

**核心方案**：系统对每个制度（Regime）实时估计其特征均值与方差，再把当前观测映射为后验概率。它不是“预测明天涨跌”，而是在问：“以今天之前的全部历史看，当前这组宏观 DNA 最像哪种制度？”

```mermaid
graph TD
    A[历史宏观 DNA] --> B{JIT GaussianNB 拟合}
    B --> C[生成 regime 均值/方差]
    C --> D[当前宏观观测向量 x]
    D --> E[计算各 regime 似然]
    E --> F[递归贝叶斯算子处理]
...
| 参数 | 物理意义 | 系统逻辑作用 |
| :--- | :--- | :--- |
| **$\mu_R$** | 制度均值 | **“指纹中心”**。描述某个制度最典型的宏观坐标。 |
| **$\sigma_R^2$** | 制度方差 | **“容错宽度”**。决定当前观测偏离制度中心后，可信度会衰减得多快。 |
| **JIT Re-fit** | 因果重训 | **“遗忘未来”**。每个窗口只使用 `T < t` 的历史数据，杜绝后视镜污染。 |

### 9.2 马氏距离几何防护：协方差矩阵对“逻辑背离”的侦测

这是 PCA 的“亲兄弟”逻辑，用于探测宏观环境是否发生了**结构性背离**（如：流动性暴跌但资产价格却非理性上涨）。

**核心方案**：系统计算当前宏观向量与“稳定核心区”的**马氏距离 (Mahalanobis Distance)**。距离越远，说明当前宏观组合越偏离历史上“正常世界”的相关结构，系统会把这种几何不适感转成连续风险折价，而不是硬阈值清仓。

```mermaid
graph LR
    A[历史牛市核心区] --> B((协方差矩阵))
    B --> C{当前组合探测}
    C -- 逻辑符合规律: 距离近 --> D[维持常规风险定价]
    C -- 指标逻辑背离: 距离远 --> E[触发连续离群惩罚]
    style B fill:#bbf,stroke:#333
```
