...
| 参数 | 物理意义 | 系统逻辑作用 |
| :--- | :--- | :--- |
| **$H_{min}$** | 完美置信度阈值 | **“安全线”**。在此熵值下，系统认为视野清晰，不执行任何削减。 |
| **$H_{max}$** | 混乱临界点 | **“警报线”**。在此熵值上，系统强制认为目前处于“不可知”状态，执行最大惩罚。 |
| **$\alpha$** | 处罚功率因子 | **“防御烈度”**。值越大，系统对“迷茫”的惩罚越呈指数级增长，表现得更保守。 |
...
$$SI = \frac{P(R_{new})}{P(R_{old}) + \delta} > \theta_{barrier}$$

| 参数 | 物理意义 | 系统逻辑作用 |
| :--- | :--- | :--- |
| **$\delta$** | 极小偏置项 | **“零点稳定器”**。数学除零保护，并提供微小的启动阻力。 |
| **$\theta_{barrier}$** | 判据壁垒 (Sigma) | **“决策能量阈值”**。类似于高保真音响的“门限”，抹除低分贝的背景噪音。 |
...
系统使用的是**自适应分位数映射 (Adaptive Quantile Mapping)**。它会问：“今天的 200 点，在过去 25 年的长河中，属于百分之几的典型水平？”

### 6.1 宏观“厚尾”处理逻辑

```mermaid
graph LR
    A[原始指标: 利差 500bps] --> B{AQM 映射}
    B -- 相比 25 年历史 --> C[分位: 0.98]
    C --> D[似然推断]
    style B fill:#bbf,stroke:#333
```

这种**“非参数记忆”**让系统具备了自我演化的能力，无论通缩还是高通胀时代，逻辑坐标系依然精准。

### 6.2 自适应分位数映射 (Adaptive Quantile Mapping)

$$F(x_t; \lambda) = \frac{\sum_{i=0}^t e^{-(t-i)\lambda} \cdot \mathbb{I}(x_i \le x_t)}{\sum_{i=0}^t e^{-(t-i)\lambda}}$$

| 参数 | 物理意义 | 系统逻辑作用 |
| :--- | :--- | :--- |
| **$\lambda$** | 时间衰减指数 | **“健忘境系数”**。控制系统对历史数据的关注权重。值越大，系统越倾向于“遗忘”过于久远的异常。 |
| **$\mathbb{I}$** | 指示函数 | **“计数器”**。统计有多少个历史日期的指标比今天更“健康”。 |
