...
*   **JIT 训练架构**：生产容器不依赖任何预训练权重。
    - **Bootstrap**：启动时从 Vercel Storage 同步 `macro_historical_dump.csv`、`qqq_history_cache.csv` 与可变 prior。
...
*   **新约束**：生产与审计冷启动必须依赖 canonical DNA 与 checked-in cold-start seed；若 `macro_historical_dump.csv`、`qqq_history_cache.csv`、regime DNA 或 cold-start seed 缺失，系统直接 fail closed，不再偷偷生成 synthetic baseline。
...
*   **审计透明度 (AC-4)**：所有导出至 `status.json` 的信号必须强制解耦 **Posterior Surface (后验真相源)** 与 **Execution Surface (执行层状态)**。
    - **Raw Target Beta**：贝叶斯核的原始科学推断结果。
    - **Target Beta**：经过惯性映射与行为守卫后的实际执行目标。
    - **Posterior Regime / Execution Regime**：前端与审计展示后验主分布，执行层状态单独透传。
    - **Deployment State / Execution Bucket**：增量节奏与存量执行桶必须分离。
    - **分发路径**：`status.json` 与 `history.json` 必须先落地到本地 `src/web/public/`，再由 `push_payload` 同步到当前 Blob 命名空间根路径，例如生产环境的 `prod/status.json` 与 `prod/history.json`。
