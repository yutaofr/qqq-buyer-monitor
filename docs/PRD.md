...
构建一个四层嵌套的过滤与确认逻辑：
*   **Tier 0**: 宏观生死线（信用利差、股权风险溢价）。详见 [宏观基本面参考手册](./fundamental_macro_v3.md)。
*   **Tier 1**: 情绪与动能自适应引擎（带 Z-Score 补偿）。详见 [自适应与流动性参考手册](./adaptive_liquidity_v4.md)。
*   **Tier 1.5**: 价值补偿与背离红利（估值、分析师修正、短线流向）。详见 [技术背离参考手册](./divergence_features.md)。
