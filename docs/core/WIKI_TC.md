# 邏輯生存：QQQ 決策系統的週期哲學與可視化指揮手冊 (v14.0 序號校準全量版)
...
在 v14 中，我們採用了雙路影子模型架構，稱為「泥地拖拉機與側車 (Mud Tractor & Sidecar)」：
...
v14 的第一件事不是「辨識某個指標」，而是判斷目前總經組合屬於哪一種**經濟-風險制度態**。

### 1.1 什麼是 REGIME，為什麼不是「另一個因子」
在本文裡，`regime` 指的是**經濟-風險制度態**。它不是輸入變數，而是系統對「目前總經物理狀態」的**壓縮標籤**。
...
1. **它在幹什麼？** 像一個有經驗的風控經理。平時看很多指標，但只有獨立方向都變壞時，才會明顯降低掛車負載。
2. **為什麼要正交？** 防止同一份證據聽兩次。剔除重複，只聽「新資訊」。
3. **為什麼熵高時要減倉？** 因為當系統自己都不確定時，最糟的動作是瞎猜。縮小部位，先活下來。
4. **為什麼準確率不如 v11.5？** 因為 v14 去掉了虛假的確定性，雖然數字看上去沒那麼亮眼，但更接近真實世界。

---

## 8. 儀表板指揮官手冊：UI 卡片全解析

當你打開 `index.html` 儀表板時，你是在監測這套重型機械的**運作完整性**。
...
v14 的哲學不再是「神諭」，而是**「耦合的藝術」**。它承認市場有迷霧，承認引擎會打滑。MT&T 架構確保了系統在泥地中不會因幻覺而翻車。
