...
* **特征契约**：当前生产配置会被序列化并计算 `sha256` 指纹，写入 `regime_audit.json`。任何特征集合或参数漂移都必须触发 fail closed，而不是静默继续推断。
...
$H(P) = -\sum_{k \in \mathcal{R}} p_k \log_2 p_k$。
...
* **数据质量惩罚**：若实时输入来自降级代理源（如 `proxy:nfci` / `proxy:hyg`），系统会先根据 `quality_score` 提高有效熵，再执行 haircut。代理数据可以参与生存性推断，但不能享有 canonical DNA 同等级别的置信度。
