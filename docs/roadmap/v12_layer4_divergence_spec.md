...
定义全局历史惊奇度的 99 分位数：$S_{99th} = \text{Percentile}(S_{micro, 0...t}, 99\%)$
**修正后的有效惊奇惩罚 (Effective Surprisal Penalty)**:
$$Penalty_t = \exp\left( \max(S_{micro, t} - Baseline_t, \ S_{micro, t} - S_{99th}) \right)$$
...
**Z-Score 归一化**:
$$Z\_Signal_t = \frac{Raw\_Signal_t - \mu_{signal}}{\sigma_{signal}}$$
...
- **WFO 机制**: 
  1. 使用 $T_{-3}$ 至 $T_0$ 的三年数据进行参数优化（Maximize IR）。
