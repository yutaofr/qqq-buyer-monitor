...
### 2.2 似然度平滑与贝叶斯完备性 (Bayesian Integrity & Tau Calibration)
*   **贝叶斯逻辑**: 废除错误的线性加权更新，全面回归 $Posterior \propto Prior \times Likelihood$ 的**乘积更新**，确保置信度能够随证据有效累积。
*   **温度平滑 (Tau)**: 针对 Naive Bayes 的“过度自信”缺陷，应用 $\tau=3.0$ 的全局温度平滑系数（或根据 `v13_4_weights_registry.json` 进行非对称映射），确保后验概率分布合理。
*   **动态先验**: 移除静态历史先验的强力拖拽，将 40% 的静态权重降至 5%，优先响应“昨日记忆”与“马尔可夫转移预测”。
