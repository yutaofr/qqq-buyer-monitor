# ADD: v8.1 线性指令链资产配置系统实现方案

> **Current main note:** the implementation described here has been merged, and the runtime now includes the dual signal-audit workflow plus the refined deployment inputs used by the current `main` branch. Merge commit: `020da23`.
>
> **Archive note:** the implementation steps below describe the baseline v8.0/v8.1 redesign. They are kept as a historical record of the merge, not as a live task list.

> 版本: v8.1
> 状态: Final
> 日期: 2026-03-25
> 对应 SRD: `docs/v8.0_linear_pipeline_srd.md` (v8.1)
...
| `test_deployment_controller.py` | Tier-0 软约束默认 ceiling; `_TIER0_CAPITULATION_OVERRIDE_THRESHOLD` 独立突破; CRISIS 不可突破 |
...
| Tier-0 软约束突破 | capitulation >= 0.7时突破到 BASE |
...
| AC-16: 移除 Cash 管理 | 系统不再预判余额，逻辑纯净化 |
...
### 13.1 DCA 择时逻辑验证
系统在 `src/backtest.py` 中实现了对增量资金 (`reserve_cash`) 的独立模拟。

- **验证工具**: `scripts/plot_dca_performance.py`
- **结论**: 在 `RICH_TIGHTENING` 和 `CRISIS` 期间，`Deployment Controller` 成功识别风险并放慢/停止部署，形成了对机械 DCA 的显著超额收益。

![DCA Performance](images/v8.1_dca_performance.png)
