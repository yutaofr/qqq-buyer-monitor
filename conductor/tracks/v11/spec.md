...
### FR-7 动量感知 (Momentum-Aware Inference)
系统必须同时感知特征的“水位”与“速度”：
1. 贝叶斯似然模型 (KDE) 必须同时处理 `_pct` (Percentile Rank) 与 `_momentum` (Z-Score of Change) 特征。
2. 特征载荷 (PCA Loadings) 必须在水位与导数之间保持相对均衡（偏差 < 30%），确保环境转折点的快速识别。
3. ERP 因子必须显式包含导数项，以区分“极值修复”与“危险加速”两种本质不同的情境。
