...
`qqq-monitor` 是一款高精度的市场配置引擎，旨在为 QQQ ETF 提供长期的动态仓位管理建议。系统不再仅仅输出单一的“买入信号”，而是通过多层级、白盒化的架构，实时输出 `allocation_state`（配置状态）以指导资金的进场节奏。系统整合了宏观经济数据、市场情绪、机构资金流以及期权市场结构。
