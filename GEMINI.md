...
- **Environment & Credentials**: All FRED API keys required for backtesting and data retrieval MUST be sourced from the `.env` file. Do not hardcode keys or use external secrets during local development or backtests. Docker compose is located in "/opt/homebrew/bin/docker compose"
