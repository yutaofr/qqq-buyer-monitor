    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Attempt {attempt+1} failed for {series_id}: {response.status_code}")
...
def run_hydration():
    ...
    master_df = pd.read_csv(BASE_DATA_PATH, parse_dates=['observation_date']).set_index('observation_date')
