...
def plot_regime_diagnostic(df, output_path, title_suffix=""):
    ...
    axes[0].set_title(f"Bayesian Regime Posterior Probabilities{title_suffix}", fontsize=18, fontweight='bold')
    ...
    for i, r in enumerate(regimes):
        mask = (df['actual_regime'] == r)
        if mask.any():
            axes[1].bar(df.index[mask], 1, width=bar_width, color=colors[i],
                        align='center', linewidth=0, alpha=1.0)
    ...
    ax_entropy.plot(plot_dates, df['entropy'], color='#34495e', linewidth=1.5, alpha=0.6, label='Info Entropy')
...
def run_visualization_suite(audit_csv="artifacts/v12_audit/full_audit.csv"):
    ...
    plot_regime_diagnostic(df, "artifacts/diagnostics/expected_vs_posterior_panorama.png", " (Panorama)")
    ...
    for name, (start, end) in slices.items():
        ...
        if not sub_df.empty:
            ...
            plot_regime_diagnostic(sub_df, f"artifacts/diagnostics/slice_{name}.png", f" ({name.replace('_', ' ').title()})")
