...
def debug():
    ...
    for i in range(1, len(df)):
        ...
        risk_relief = risk_delta <= -0.02 or (
            previous_combined_risk >= 0.30 and combined_risk <= 0.18 and risk_delta <= -0.12
        )
        entropy_waterfall = (
            previous_effective_entropy >= 0.55
        ...
        if risk_clear and risk_relief and mid_cycle_surge and bust_retreat:
            ...
            if not entropy_waterfall:
                print(
                    f"[{row['date']}] BUY NEAR MISS (Entropy Waterfall Failure): entropy={effective_entropy:.3f}, prev_entropy={previous_effective_entropy:.3f}, delta={entropy_delta:.3f}"
                )
            else:
                print(
                    f"[{row['date']}] BUY SIGNAL DETECTED in debug script! Flags: rc={risk_clear}, rr={risk_relief}, ew={entropy_waterfall}, mcs={mid_cycle_surge}, br={bust_retreat}"
                )
