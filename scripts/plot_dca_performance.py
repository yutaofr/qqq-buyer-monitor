...
def plot_dca_performance():
    ...
    summary = tester.simulate_portfolio(
        qqq, seeder, enable_dynamic_search=True, registry_path="data/candidate_registry_v7.json"
    )
    ...
    ax1.plot(
        daily_ts.index,
        daily_ts["nav"],
        label="v8.1 Tactical Model (DCA)",
        color="#00ff9d",
        linewidth=1.5,
    )
    ax1.plot(
        daily_ts.index,
        daily_ts["baseline_nav"],
        label="Baseline Benchmark (DCA)",
        color="#ff3366",
        linewidth=1.5,
        alpha=0.7,
    )
    ...
    ax1.set_title(
        f"v8.1 QQQ Add-Timing Performance vs Baseline DCA\nAvg Cost Improvement: {summary.average_cost_improvement_vs_baseline_dca * 100:.2f}% | Signal Beta: {summary.signal_beta:.2f}",
        fontsize=14,
        pad=15,
    )
    ...
    ax2.fill_between(
        daily_ts.index, 0, est_reserve_cash, color="#00ccff", alpha=0.3, label="Reserve Cash Buffer"
    )
    ...
    ax2.scatter(
        pauses.index,
        (pauses["reserve_cash_pct"] / 100.0 * pauses["nav"]),
        color="#ff9900",
        s=10,
        label="Deploy Paused (Crisis/Rich)",
        zorder=5,
    )

    ax2.set_title(
        "Reserve Cash Deployments (100% Invested indicates $0 Balance)", fontsize=12, pad=10
    )
