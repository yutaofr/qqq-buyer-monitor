...
def render_full_panorama(
    ...
    axes[6].plot(
        ...
        linestyle="--",
        alpha=0.6,
        label="Raw Bayesian Beta",
    )
    axes[6].plot(
        plot_dates, df_betas["target"], color="#2c3e50", linewidth=3, label="Final Target Beta"
    )
    axes[6].set_title(
        "Panel 6: Beta Surface Dynamics (Strategic De-risking)", fontsize=18, fontweight="bold"
    )
...
def run_backtest_simulation(start_date="2018-01-01", end_date="2026-04-09"):
    logger.info(
        f"V22-ULTIMATE: Running full 8-year forensic simulation from {start_date} to {end_date}..."
    )
    ...
    regime_df = pd.read_csv(
        "data/v11_poc_phase1_results.csv", parse_dates=["observation_date"]
    ...
    for dt in test_dates:
        ...
        try:
            ...
            if not raw_z:
                raw_z = {
                    k: v
                    for k, v in runtime.get("feature_values", {}).items()
...
def run_panorama_visualization(force_simulation=False):
    ...
    df_radar = df[[c for c in df.columns if "radar_" in c]].rename(
        columns=lambda x: x.replace("radar_", "")
    )
    series_entropy = df["entropy"]

    df_betas = pd.DataFrame(
        {"raw": df["raw_target_beta"], "target": df["target_beta"]}, index=df.index
    )
    ...
    render_full_panorama(
        ...
        df_signals,
        baseline_results,
        _get_close_series(qqq_raw),
        _get_close_series(spy_raw),
