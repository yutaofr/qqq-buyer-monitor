...
def test_kelly_fraction_negative():
    ...
    posteriors = {"BUST": 1.0}
    k = compute_kelly_fraction(
        posteriors=posteriors, regime_sharpes=DEFAULT_SHARPES, entropy=0.0, erp_percentile=0.5
    )
    assert math.isclose(k, -1.0)
...
def test_kelly_fraction_with_entropy():
    ...
    k = compute_kelly_fraction(
        posteriors=posteriors, regime_sharpes=DEFAULT_SHARPES, entropy=0.3, erp_percentile=0.5
    )
...
def test_kelly_fraction_with_erp():
    ...
    # raw = 0.1 * 1.2 / 0.81
    k = compute_kelly_fraction(
        posteriors=posteriors, regime_sharpes=DEFAULT_SHARPES, entropy=0.0, erp_percentile=1.0
    )
...
def test_kelly_fraction_clipping_inputs():
    ...
    k = compute_kelly_fraction(
        posteriors=posteriors, regime_sharpes=DEFAULT_SHARPES, entropy=1.5, erp_percentile=-1.0
    )
