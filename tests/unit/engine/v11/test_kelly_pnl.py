...
from scripts.kelly_pnl_backtest import _compute_performance_metrics, _compute_pnl_curve


def test_pnl_flat_market_zero_return():
    """价格不变时，净值接近 1.0（仅扣交易成本）"""
    trace = pd.DataFrame({"close": [100.0, 100.0, 100.0], "beta": [1.0, 1.0, 1.0]})
    nav = _compute_pnl_curve(trace, "beta", transaction_cost=0.0)
...
def test_pnl_higher_beta_amplifies_return():
    """更高的 beta 在上涨市场中产生更高净值"""
    trace = pd.DataFrame(
        {"close": [100.0, 105.0, 110.25], "beta_low": [0.5, 0.5, 0.5], "beta_high": [1.5, 1.5, 1.5]}
    )
    nav_low = _compute_pnl_curve(trace, "beta_low", transaction_cost=0.0)
    nav_high = _compute_pnl_curve(trace, "beta_high", transaction_cost=0.0)
