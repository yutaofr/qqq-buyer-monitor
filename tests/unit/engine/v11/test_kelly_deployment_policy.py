...
def test_decide_holds_state_when_evidence_low():
    ...
    res2 = policy.decide(
        posteriors={"RECOVERY": 1.0}, entropy=0.8, readiness_score=0.5, value_score=0.5
    )
...
def test_decide_switches_state_when_evidence_high():
    ...
    res = policy.decide(
        posteriors={"RECOVERY": 1.0}, entropy=0.5, readiness_score=0.5, value_score=0.5
    )
...
def test_decide_resets_evidence_on_same_raw_state():
    ...
    res = policy.decide(
        posteriors={"LATE_CYCLE": 1.0}, entropy=0.5, readiness_score=0.5, value_score=0.5
    )
...
def test_decide_return_keys():
    ...
    policy = KellyDeploymentPolicy()
    res = policy.decide(
        posteriors={"MID_CYCLE": 1.0}, entropy=0.0, readiness_score=0.2, value_score=0.3
    )
    expected_keys = {
        "deployment_state",
        "raw_state",
